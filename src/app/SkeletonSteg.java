...
class Steg {
	 ...
	 *            - the filename of the cover image as a string
	 * @param payload
	...
	public String hideString(String payload, String cover_filename) {
		List<Integer> payloadAsBits = stringToBits(payload);
		List<Integer> coverFileAsBits = imagetoBits(cover_filename);
		List<Integer> stegoImageAsBits;
		try{
			stegoImageAsBits = hidePayloadBitsInImage(payloadAsBits, coverFileAsBits);//will use swapLSB
		}
		catch(Exception e){
			return "Fail";
		}
		String stegoFileName = bitsToImage(stegoImageAsBits);//creates image file and returns its name
		return stegoFileName;
	}
	...
	public String extractString(String stego_image) {
		List<Integer> stegoImageAsBits = imageToBits(stego_image);//same one as you used in hideString
		List<Integer> payloadAsBits;
		try{
			payloadAsBits = extractPayloadBitsFromImage(stegoImageAsBits);
		}
		catch(Exception e){
			return "Fail";
		}
		String payload = bitsToString(payloadAsBits);
		return payload;
	}
	 ...
	 * 
	 * @param file_payload
	 *            - the name of the file to be hidden, you can assume it is in
	 *            the same directory as the program
	 * @param cover_image
	 *            - the name of the cover image file, you can assume it is in
	 *            the same directory as the program
	...
	// TODO you must write this method
	/**
	 * This method swaps the least significant bit with a bit from the
	 * filereader
	 * 
	 * @param bitToHide
	 *            - the bit which is to replace the lsb of the byte of the image
	 * @param byt
	 *            - the current byte
